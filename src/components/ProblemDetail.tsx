...
export default function ProblemDetail() {
  ...
  const [activeTab, setActiveTab] = useState<"descriere" | "trimiteri">("descriere");
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [userRes, probRes] = await Promise.all([
        fetch("https://localhost:7148/api/user/getUser", {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
          body: JSON.stringify(userDto),
        }),
        fetch(`https://localhost:7148/api/problem/getProblem/${id}`, {
          method: "GET",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        })
      ]);
      ...
      console.error("Eroare Inițializare:", error.message);
    }
  };
  ...
  const createSubmissionPayload = (): AddSubmissionDTO | null => {
    ...
    if (!userData) return null; // Am scos verificarea de admin pentru a permite tuturor să trimită, ajustează dacă e necesar
  ...
  const handleAction = async (endpoint: string) => {
    ...
    if (!payload) return setConsoleOutput(["Eroare autentificare: Te rugăm să te reconectezi."]);

    setIsRunning(true);
    setConsoleOutput([`${endpoint === 'runCode' ? 'Se execută' : 'Se trimite'}...`]);

    try {
      // Ajustat portul la 7148 pentru consistență
      const response = await fetch(`https://localhost:7148/api/compiler/${endpoint}`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      ...
      setConsoleOutput(["Eroare", err.message]);
    } finally {
      setIsRunning(false);
    }
  };
      ...
      <header className="bg-white border-b border-gray-200 px-4 py-2 flex items-center justify-between shadow-sm z-10">
        ...
        <div className="flex items-center space-x-3 text-sm">
          <div className={`px-3 py-1 rounded-full font-medium ${problem?.acceptanceRate && problem.acceptanceRate > 50 ? 'bg-emerald-50 text-emerald-600' : 'bg-orange-50 text-orange-600'}`}>
            Rată de succes: {problem?.acceptanceRate}%
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 flex overflow-hidden">
        <section className="w-1/2 flex flex-col bg-white border-r border-gray-200">
          <div className="flex border-b border-gray-100 px-4">
            {(["descriere", "trimiteri"] as const).map((tab) => (
              <button
                key={tab}
          ...
          <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
            {activeTab === "descriere" ? (
              <div className="max-w-3xl space-y-8 animate-in fade-in duration-300">
                ...
                <div className="space-y-4">
                  <h3 className="flex items-center text-sm font-bold text-gray-900 uppercase tracking-wider">
                    <Info className="w-4 h-4 mr-2 text-emerald-500" /> Exemple
                  </h3>
                    ...
                    {problem?.inputsJson?.map((input, idx) => (
                      <div key={idx} className="bg-gray-50 rounded-xl p-4 border border-gray-100 font-mono text-xs">
                        <div className="grid grid-cols-12 gap-2">
                          <span className="col-span-2 text-gray-400 font-bold uppercase">Intrare</span>
                          <span className="col-span-10 text-gray-800 bg-white p-1 rounded border border-gray-200">{input}</span>
                          <span className="col-span-2 text-gray-400 font-bold uppercase">Ieșire</span>
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                <h3 className="text-lg font-bold text-gray-900">Trimiteri Recente</h3>
                <div className="space-y-3">
                  {submissions.filter(s => s.problemId === id).length > 0 ? (
                                ...
                                <div>
                                  <div className="flex items-center space-x-2">
                                    <span className={`font-bold text-sm ${isAccepted ? 'text-emerald-600' : 'text-red-600'}`}>
                                      {isAccepted ? "Acceptat" : "Răspuns Greșit"}
                                    </span>
                    ...
                    <div className="text-center py-20 text-gray-400 italic">Nicio trimitere momentan.</div>
                  )}
                </div>
              </div>
            )}
          ...
          <div className="h-12 bg-[#252526] border-b border-black/20 flex items-center justify-between px-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              >
                ...
                <span className="text-xs font-bold uppercase">Rulează</span>
              </button>
              <button 
              ...
              >
                ...
                <span className="text-xs font-bold uppercase tracking-wide">Trimite</span>
              </button>
            </div>
          </div>
          ...
          <div className="flex bg-[#252526] overflow-x-auto no-scrollbar border-b border-black/20">
            ...
            <button 
               onClick={() => {
                 ...
                 setFiles([...files, { id: newId, name: `modul_${files.length}.c`, content: "" }]);
            ...
            <div className="p-4 overflow-y-auto h-[calc(100%-32px)] custom-scrollbar">
              {consoleOutput.map((line, i) => (
                <div key={i} className="font-mono text-[11px] text-gray-400 mb-1 flex animate-in fade-in slide-in-from-left-2">
                  ...
                  <span className={line.includes("Eroare") || line.includes("failed") ? "text-red-400" : ""}>{line}</span>
                </div>
              ))}
              {consoleOutput.length === 0 && <div className="text-gray-600 font-mono text-[11px] italic">Se așteaptă execuția...</div>}
            </div>
          </div>
        </section>
      </main>
