...
export default function CreateProblem() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const allInputsValid = formData.inputsJson.every(isValidJson);
    const allOutputsValid = formData.outputsJson.every(isValidJson);

    if (!allInputsValid || !allOutputsValid) {
      alert("Te rog corectează erorile JSON din test case-uri.");
      return;
    }

    setIsSubmitting(true);
    try {
      const response = await fetch("https://localhost:7148/api/admin/addProblem", {
        method: "POST",
        headers: { 
          "Content-Type": "application/json", 
          Authorization: `Bearer ${token}` 
            ...
            <input 
              ...
              onKeyDown={(e) => {
                if(e.key === 'Enter') {
                  ...
                  if(val) {
                    setFormData(prev => ({ ...prev, tags: [val, ...prev.tags] }));
